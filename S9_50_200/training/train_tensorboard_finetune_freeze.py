...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-file', type=str, default=r'D:/Research_data/RCAN_HXN/RCAN_official/Natural_2_8_pystack_align_submean_finetune/DIV_fly_scan_gray_finetune.h5')
    parser.add_argument('--eval-file', type=str, default=r'D:/Research_data/RCAN_HXN/RCAN_official/Natural_2_8_pystack_align_submean_finetune/DIV_fly_scan_gray_eval_finetune.h5')
    parser.add_argument('--outputs-dir', type=str, default=r'./outputs_scale_2_8_l1_finetune_submean_freeze2')
    ...
    # load the model and freeze previous layers
    ...
    for epoch in range(args.num_epochs):
        ...
        # if epoch > args.num_epochs * 0.67:
        if epoch == 400:
            for n, para in model.named_parameters():
                print('allow all grad')
                para.requires_grad = True

        model.train()
        epoch_losses = AverageMeter()

        with tqdm(total=(len(train_dataset) - len(train_dataset) % args.batch_size), ncols=90) as t:
    ...
    np.save('log_loss_finetune.npy', log_loss)
    np.save('log_psnr_finetune.npy', log_psnr)
